...
output_dir = r"D:\HUST-MI2\2025.2\Đồ án 1-Thầy Đoàn Duy Trung\Model_Output\KMeans_Truoc_Ngoai_Lai"
...
print("\n========== DỮ LIỆU KHÁCH HÀNG SAU TỔNG HỢP ==========")
...
# 6. CHỌN SỐ CỤM BẰNG ELBOW METHOD VÀ SILHOUETTE SCORE
...
plt.title("Elbow Method - KMeans trước xử lý ngoại lai")
plt.grid(True)
save_fig("kmeans_before_elbow_method.png")

plt.figure(figsize=(8, 5))
plt.plot(list(k_values), silhouette_scores, marker="o")
plt.xlabel("Số cụm K")
plt.ylabel("Silhouette Score")
plt.title("Silhouette Score - KMeans trước xử lý ngoại lai")
plt.grid(True)
save_fig("kmeans_before_silhouette_score.png")


# 7. HUẤN LUYỆN K-MEANS VỚI SỐ CỤM ĐƯỢC CHỌN

kmeans_final = KMeans(n_clusters=best_k, random_state=42, n_init=10)
customer_df["Cluster"] = kmeans_final.fit_predict(X_scaled)


# 8. TÓM TẮT ĐẶC ĐIỂM TỪNG CỤM
...
plt.title("Phân cụm khách hàng theo doanh thu và lợi nhuận - trước xử lý ngoại lai")
plt.grid(True)
save_fig("kmeans_before_customer_clusters_sales_profit.png")

plt.figure(figsize=(8, 5))
plt.bar(cluster_summary["Cluster"].astype(str), cluster_summary["Customer_Count"])
plt.xlabel("Cụm khách hàng")
plt.ylabel("Số lượng khách hàng")
plt.title("Số lượng khách hàng trong từng cụm - trước xử lý ngoại lai")
plt.grid(True)
save_fig("kmeans_before_customer_count_by_cluster.png")
...
plt.title("Doanh thu và lợi nhuận trung bình theo cụm - trước xử lý ngoại lai")
plt.xticks(x, cluster_summary["Cluster"].astype(str))
plt.legend()
plt.grid(True)
save_fig("kmeans_before_avg_sales_profit_by_cluster.png")


# 10. LƯU KẾT QUẢ

customer_df.to_csv(os.path.join(output_dir, "kmeans_before_customer_clusters.csv"), index=False, encoding="utf-8-sig")
cluster_summary.to_csv(os.path.join(output_dir, "kmeans_before_cluster_summary.csv"), index=False, encoding="utf-8-sig")
evaluation_df.to_csv(os.path.join(output_dir, "kmeans_before_evaluation_k.csv"), index=False, encoding="utf-8-sig")

best_row = evaluation_df[evaluation_df["K"] == best_k].iloc[0]

metrics_df = pd.DataFrame({
    "Model": ["KMeans"],
    "Case": ["Trước xử lý ngoại lai"],
    "Best_K": [best_k],
    "Best_K_WCSS_Inertia": [best_row["WCSS_Inertia"]],
    "Best_K_Silhouette_Score": [best_row["Silhouette_Score"]]
})

metrics_df.to_csv(os.path.join(output_dir, "kmeans_before_metrics.csv"), index=False, encoding="utf-8-sig")
