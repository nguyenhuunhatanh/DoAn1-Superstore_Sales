...
plt.title("Random Forest Regressor có xử lý ngoại lai: Actual vs Predicted")
plt.grid(True)
plt.show()
...
plt.xlabel("Sai số")
plt.ylabel("Tần suất")
plt.title("Phân bố sai số của Random Forest có xử lý ngoại lai")
plt.grid(True)
plt.show()
...
if len(numeric_features) > 0:
    ...
    plt.title(f"Trước xử lý ngoại lai\n{col_plot}")

    plt.subplot(1, 2, 2)
    plt.boxplot(X_capped[col_plot].dropna())
    plt.title(f"Sau xử lý ngoại lai\n{col_plot}")
...
result.to_csv("random_forest_outlier_predictions.csv", index=False, encoding="utf-8-sig")
importance_df.to_csv("random_forest_outlier_feature_importance.csv", index=False, encoding="utf-8-sig")
cap_info_X.to_csv("random_forest_outlier_capping_info_X.csv", index=False, encoding="utf-8-sig")
cap_info_y.to_csv("random_forest_outlier_capping_info_y.csv", index=False, encoding="utf-8-sig")
...
metrics_df.to_csv("random_forest_outlier_metrics.csv", index=False, encoding="utf-8-sig")

print("\nĐã lưu các file kết quả:")
print("- random_forest_outlier_predictions.csv")
print("- random_forest_outlier_feature_importance.csv")
print("- random_forest_outlier_capping_info_X.csv")
print("- random_forest_outlier_capping_info_y.csv")
print("- random_forest_outlier_metrics.csv")