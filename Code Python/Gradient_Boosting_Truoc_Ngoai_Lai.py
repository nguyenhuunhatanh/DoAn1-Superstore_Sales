...
output_dir = r"D:\HUST-MI2\2025.2\Đồ án 1-Thầy Đoàn Duy Trung\Model_Output\Gradient_Boosting_Truoc_Ngoai_Lai"
...
# Nếu dự đoán Profit thì bỏ Sales để mô hình khách quan hơn
...
# 5. TẠO MÔ HÌNH GRADIENT BOOSTING
...
plt.title("Gradient Boosting trước xử lý ngoại lai: Actual vs Predicted")
plt.grid(True)
save_fig("gradient_boosting_before_actual_vs_predicted.png")
...
plt.xlabel("Sai số")
plt.ylabel("Tần suất")
plt.title("Phân bố sai số của Gradient Boosting trước xử lý ngoại lai")
plt.grid(True)
save_fig("gradient_boosting_before_residual_distribution.png")
...
plt.ylabel("Biến")
plt.title("Top 10 biến quan trọng nhất - Gradient Boosting trước xử lý ngoại lai")
plt.grid(True)
save_fig("gradient_boosting_before_feature_importance.png")


# 10. LƯU KẾT QUẢ
result.to_csv(os.path.join(output_dir, "gradient_boosting_before_predictions.csv"), index=False, encoding="utf-8-sig")
importance_df.to_csv(os.path.join(output_dir, "gradient_boosting_before_feature_importance.csv"), index=False, encoding="utf-8-sig")
...
metrics_df.to_csv(os.path.join(output_dir, "gradient_boosting_before_metrics.csv"), index=False, encoding="utf-8-sig")
